...
public class SrvCheckbooksV01 implements ISrvCheckbooksV01, com.bbva.jee.arq.spring.core.servicing.utils.ContextAware {
	...
	//	new FiqlValidator(filter).exist().hasGeAndLe("month").validate();
		...
		DTOIntCheckFilter dtoIntExecutivesFilter = checkbookMapper.getDTOIntFilter(checkId,accountId);

		// 3. Invoke SrvIntAccounts and Mapping to canonical DTO
		return checkbookMapper.map(srvIntCheckbooks.getChecks(dtoIntExecutivesFilter));
	}
