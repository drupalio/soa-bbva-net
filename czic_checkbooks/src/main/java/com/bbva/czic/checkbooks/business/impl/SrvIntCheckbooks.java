...
public class SrvIntCheckbooks implements ISrvIntCheckbooks {
		...
		try {
			...
			if (filter != null && !filter.contentEquals("null")) {
				...
				try {
					...
					ch = new FiqlParser<DTOIntCheck>(DTOIntCheck.class)
							.parse(filter);
					// pm =new FiqlParser
					// <DTOIntCheck>(DTOIntCheck).parse(status);
					// final List<DTOIntEnumCheckStatus> splitDataFilter1 =
					// bussinesToolKit.getDataFromFilter(ch);
					// for (DTOIntEnumCheckStatus st: SplitFilter1) {
					// status = st.get
					// }

					final List<PrimitiveStatement> splitDataFilter = bussinesToolKit
							.getDataFromFilter(ch);
					for (PrimitiveStatement st : splitDataFilter) {
						proper = st.getProperty();
						value = st.getValue();
						condition = st.getCondition();

						if (proper.equals("issueDate")
								&& condition.equals(ConditionType.GREATER_THAN)) {
							fechini = (Calendar) st.getValue();

						} else if (proper.equals("issueDate")
								&& condition
										.equals(ConditionType.LESS_OR_EQUALS)) {
							fechfin = (Calendar) st.getValue();
						} else if (proper.equals("status")
					 ...
					 * bussinesToolKit.matchesPatternFromFilter(SERVICENAME,
					 * filter); if (pattern != null) {
				 ...
				 * return found;
				 */
				List<DTOIntCheck> dtoIntChecks = checkbookDao.getChecks(id,
						fechini, fechfin, status, indpagi, tampagi);
	 ...
	 * 
	 * @see
	 * com.bbva.czic.checkbooks.business.ISrvIntCheckbooks#getCheckbooks(java
	 * .lang.String)
	 */
	...
	public List<Checkbook> getCheckbooks(String id)
		...
		try {
			if (id == null) {
				throw new BusinessServiceException("wrongParametersCheckbooks");
			}

			List<Checkbook> listCheckbooks = new ArrayList<Checkbook>();
			checkbookDao.getCheckbooks(id);
			List<DTOIntCheckbook> dtoIntCheckbook = checkbookDao
					.getCheckbooks(id);
